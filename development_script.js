...
if (_loadersList.length) {
    let containerHtml = '';
    for (var i = 0; i < _loadersList.length; i++) {
        containerHtml += `<div class="col-sm-3 loader-box viewloader" style="min-height: 150px; margin-bottom: 15px;" id="loader${i}">
        <div class="card"><div class="card-body" style="min-height: 92px;">${_loadersList[i]}</div>
        <div class="card-footer" style="font-weight: 600;">Loader No - ${i}<a href="javascript:void(0);" class="viewloader" id="loader${i}">[--]</a></div></div></div>`;
    }
    $('#loaderList').html(containerHtml);
}
...
$(document).ready(function() {
    ...
    if(urlParams.has('loader')){
        var lno = urlParams.get('loader').split('loader');
        if (lno[1] <= _loadersList.length) {
            $('#arindamLoaderInner').html(_loadersList[lno[1]]);
        } else {
            $('#arindamLoaderInner').html(_loadersList[0]);
        }
    } else {
        $('#arindamLoaderInner').html(_loadersList[0]);
    }
